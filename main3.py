...
#=2024-04-26,11点41, 添加模糊匹配功能.我们用?来代表任意一个非空字符.
print('我们这里面用?来表示任意匹配一个非空字符.')
...
#==========现在我们把这个最短匹配当做默认情况,因为这个算法是最快的.一般也足够用了!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    def pipei_shortest_with_rep_any(self, message, repl="*", represent_any_char='?'):
        # 我们用?来代表任意一个非空字符, 这个用在字幕表keywords_ie里面.
        if not isinstance(message, str):
            message = message.decode('utf-8')
        outdex=[] #输出key索引
        outkey=[]
            ...
            for index,char in enumerate( message[start:]):
                ...
                if char in level:
                    ...
                    if self.delimit not in level[char]:#无脑进入.
                        ...
                        outcat.append(level[char][self.delimit])
                        break
                elif '?' in level: #=========这段是模糊匹配逻辑
                        tmpchar='?'
                        #如果?在模板里面. 那么我们就进入这个
                        #======因为我们匹配最短, 所以直接到问号就一定停止了.后面再匹配上也是更长.我们不要了.
                        step_ins += 1
                        old_start=start
                        start += step_ins - 1
                        outdex.append([old_start,start])
...
#最长匹配, 尽量找跟字典中最长的匹配, 尽可能让找到的字符串最长!!!!!!!!!!!!!!!!!!!性能会比上面的低很多!
    def pipei_longest_with_rep_any(self, message, repl="*",represent_any_char='?'):
        if not isinstance(message, str):
            message = message.decode('utf-8')
        outdex=[] #输出key索引
        outkey=[]
...
if 1:
        ...
        print('问号的任意匹配最短测试')
        print(gfw.pipei_shortest_with_rep_any("苹aa"))



        print('问号的任意匹配最长测试')
        print(gfw.pipei_longest_with_rep_any("苹aa"))